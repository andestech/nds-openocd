	...
	if (zero_check == 0x00 || one_check == 0xff) {
		...
		if (nds_scan_retry_times == 0) {
			if ((custom_initial_script != NULL) ||
				...
				count_retry_2wire++;
				if (zero_check == 0x00) {
					/* zero_check = 0 : aice-micro 2wire mode ,use 2wire config(reference aice_micro_sdp.cfg)
					 * and reinit adapter(jtag) */
					...
					/* zero_check = 1 : aice-mini+ 2wire mode ,use 2wire config(reference aice_sdp.cfg)
					 * and reinit adapter(jtag) */
