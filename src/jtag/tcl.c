...
static int Jim_Command_drscan(Jim_Interp *interp, int argc, Jim_Obj *const *args)
{
	...
	*     args[N-2] = "-endstate"
	...
	if (two_wire_mode) {
		...
		for (i = 2, field_count = 0; i < argc; i += 2, field_count += 1) {
			bit_copy(compressed_out, compressed_out_offset, fields[field_count].out_value,
					0, fields[field_count].num_bits);
	...
	if (two_wire_mode) {
		for (i = 2, field_count = 0, compressed_out_offset = 0; i < argc; i += 2, field_count += 1) {
			bit_copy(fields[field_count].in_value, 0, compressed_out,
					compressed_out_offset, fields[field_count].num_bits);
			compressed_out_offset += fields[field_count].num_bits;
		}
		free(compressed_out);
