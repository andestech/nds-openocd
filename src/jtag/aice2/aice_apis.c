...
uint32_t aice_ice_config;
uint32_t aice_hardware_version;
uint32_t aice_firmware_version;
uint32_t aice_fpga_version;
uint32_t aice_batch_data_buf1_size;
uint32_t force_edm_v3 = 1;
uint32_t aice_packet_append_size;
...
char *custom_initial_script;
struct target g_default_target;
struct jtag_tap g_default_tap;

/* SDM(System_Debug_Module) apis */
uint32_t aice_default_use_sdm, aice_current_use_sdm;
uint32_t aice_sdm_support, aice_sdm_support_ena;
uint32_t aice_sdm_cfg_value;
uint32_t aice_sdm_parallel_debug_bus;
...
int aice_read_edm(struct target *target, uint32_t JDPInst, uint32_t address,
		uint32_t *pEDMData, uint32_t num_of_words)
{
	uint32_t coreid = target_to_coreid(target);
	aice_sdm_direct_select(coreid);
...
int aice_write_sdm(struct target *target, uint32_t JDPInst, uint32_t address,
		uint32_t *pEDMData, uint32_t num_of_words)
{
	uint32_t coreid = target_to_coreid(target);
...
int aice_read_sdm(struct target *target, uint32_t JDPInst, uint32_t address,
		uint32_t *pEDMData, uint32_t num_of_words)
{
	uint32_t coreid = target_to_coreid(target);
	aice_write_ctrl(AICE_WRITE_CTRL_SDMCONN, NDS_SDM_SELECT_SDM);
...
int aice_write_edm_by_coreid(uint32_t coreid, uint32_t JDPInst, uint32_t address,
		uint32_t *pEDMData, uint32_t num_of_words)
{
	aice_sdm_direct_select(coreid);
	return aice_port->api->write_edm(coreid, JDPInst, address, pEDMData, num_of_words);
...
int aice_read_edm_by_coreid(uint32_t coreid, uint32_t JDPInst, uint32_t address,
		uint32_t *pEDMData, uint32_t num_of_words)
{
	aice_sdm_direct_select(coreid);
	return aice_port->api->read_edm(coreid, JDPInst, address, pEDMData, num_of_words);
...
uint32_t aice_curr_command_mode;
/*
...
int aice_run_target(struct target *target)
{
	...
	do {
		...
		if (0 == (NDS_DBGER_DEX & dbger_value))
			break;
...
#if 0
	/* do aice_edm_init when gdb connect */
	for (target = all_targets; target; target = target->next) {
		coreid = target_to_coreid(target);
		aice_edm_init(coreid);
	}
...
int aice_open_device(struct aice_port_param_s *param)
{
	...
	if (nds32_reset_halt_as_init) {
		if (nds32_reset_halt_as_init == 2) {
			/* soft_reset_halt as init */
