...
extern int aice_write_edm_by_coreid(uint32_t coreid, uint32_t JDPInst,
		uint32_t address, uint32_t *pEDMData, uint32_t num_of_words);
extern int aice_read_edm_by_coreid(uint32_t coreid, uint32_t JDPInst,
		uint32_t address, uint32_t *pEDMData, uint32_t num_of_words);
...
unsigned int usb_out_packets_buffer_length;
unsigned int usb_in_packets_buffer_length;
unsigned int aice_keep_usb_packet_append;
unsigned int aice_keep_usb_in_curr_packets_length;
...
uint32_t aice_write_pins_support;
unsigned int log_usb_packets;
...
int aice_usb_open(unsigned int usb_vid, unsigned int usb_pid)
{
	...
	*/
	...
	AICE_USBCMMD_MSG("aice_read_ep=0x%x, aice_write_ep=0x%x",
			aice_read_ep, aice_write_ep);
	AICE_USBCMMD_MSG("aice_usb_rx_max_packet=%d, aice_usb_tx_max_packet=%d",
						...
						while (1) {
							...
							*/
							if (cur_attr & 0x01)
								return rx_length;
							cur_bytes += rx_length;
							ret = f(dev, ep, (char *)cur_bytes, size - (cur_length << 2),
									timeout, &result);
						}
		...
		char *bytes, int size, int timeout)
{
	...
	if ((size % aice_usb_rx_max_packet) == 0) {
		...
		jtag_libusb_bulk_read(dev, ep, &zero_buffer[0], aice_usb_rx_max_packet, timeout, &tr);
	}
	if (log_usb_packets == 1) {
		unsigned int i;
		char msgbuffer[4096] = {0};
...
int aice_batch_buffer_write(uint32_t buf_index)
{
	...
	result = aice_access_cmmd(AICE_CMDIDX_BATCH_BUFFER_WRITE, 0, buf_index,
...
int aice_batch_buffer_read(uint32_t buf_index, unsigned char *pReadData, uint32_t num_of_words)
{
	...
	enum aice_command_mode aice_command_mode_ori = aice_command_mode;
	aice_command_mode = AICE_COMMAND_MODE_NORMAL;
	result = aice_access_cmmd(AICE_CMDIDX_BATCH_BUFFER_READ, 0, buf_index,
			(unsigned char *)pReadData, num_of_words);
	aice_command_mode = aice_command_mode_ori;
	return result;
}
...
int aice_usb_read_edm(uint32_t target_id, uint8_t JDPInst,
		uint32_t address, uint32_t *EDMData, uint32_t num_of_words)
{
	int result = ERROR_FAIL;
...
int aice_usb_write_edm(uint32_t target_id, uint8_t JDPInst,
		uint32_t address, uint32_t *EDMData, uint32_t num_of_words)
{
	int result = 0;
	...
	/*
	if( num_of_words == 1 ) {
		LOG_DEBUG("AICE Write EDM USB: target_id=0x%08X, cmd=0x%02X, addr=0x%08X, date=0x%08X",
			target_id, JDPInst, address, *EDMData);
	} else {
		LOG_DEBUG("AICE Write EDM USB: target_id=0x%08X, cmd=0x%02X, addr=0x%08X",
			target_id, JDPInst, address);
	}
	*/
...
static int aice_usb_packet_flush(void)
{
	...
	if (AICE_COMMAND_MODE_PACK == aice_command_mode) {
		...
		if (aice_usb_read(&usb_in_packets_buffer[aice_keep_usb_in_curr_packets_length],
					usb_in_packets_buffer_length) < 0) {
			AICE_USBCMMD_MSG("Flush FAIL: aice_usb_read");
			return ERROR_FAIL;
		}
		if (aice_keep_usb_packet_append == 1) {
			AICE_USBCMMD_MSG("aice_keep_usb_in_curr_packets_length: 0x%x",
					aice_keep_usb_in_curr_packets_length);
	...
	do {
		...
		if ((result < 0) ||
			  ...
			  (cmdidx != AICE_CMDIDX_SCAN_CHAIN))) {  /* scan_chain received data maybe < d2h_size */
				AICE_USBCMMD_MSG("%s, aice_usb_read failed (requested=%d, result=%d)",
						pusb_cmmd_attr->cmdname, d2h_size, result);
				return ERROR_FAIL;
		}
		...
		if (i <= AICE_CUSTOM_CMD_DELAY) {
			...
			if (result == 3) {
				if (i == AICE_CUSTOM_CMD_T_WRITE_MISC) {
					...
					LOG_DEBUG("custom_script aice_write_misc, 0x%x, 0x%x, 0x%x",
							target_id, write_misc_addr, write_misc_data);
					result = aice_write_edm_by_coreid(target_id, JDP_W_MISC_REG, write_misc_addr,
							(uint32_t *)&write_misc_data, 1);
					if (result != ERROR_OK)
						goto aice_execute_custom_script_error;
					...
					LOG_DEBUG("custom_script aice_write_edmsr, 0x%x, 0x%x, 0x%x",
							target_id, write_edmsr_addr, write_edmsr_data);
					result = aice_write_edm_by_coreid(target_id, JDP_W_DBG_SR, write_edmsr_addr,
							(uint32_t *)&write_edmsr_data, 1);
					if (result != ERROR_OK)
						goto aice_execute_custom_script_error;
				}
			}
			...
			result = sscanf(curr_str + strlen(compare_str), " %d %d %d %d",
					&argv[0], &argv[1], &argv[2], &argv[3]);
			if (result != 4) {
				result = sscanf(curr_str + strlen(compare_str), " 0x%x 0x%x 0x%x 0x%x",
						&argv[0], &argv[1], &argv[2], &argv[3]);
				if (result != 4)
			...
			if (result == 4) {
				...
				LOG_DEBUG("custom_script aice_write_edmreg, 0x%x, 0x%x, 0x%x",
						write_edm_reg, write_edm_addr, write_edm_data);
				result = aice_write_edm_by_coreid(target_id, write_edm_reg|JDP_WRITE, write_edm_addr,
						(uint32_t *)&write_edm_data, 1);
				if (result != ERROR_OK)
					goto aice_execute_custom_script_error;
			}
		} else if (i == AICE_CUSTOM_CMD_T_READ_EDMREG) {
			result = sscanf(curr_str + strlen(compare_str), " %d %d %d %d %d",
					&argv[0], &argv[1], &argv[2], &argv[3], &argv[4]);
			if (result != 5) {
				result = sscanf(curr_str + strlen(compare_str), " 0x%x 0x%x 0x%x 0x%x 0x%x",
						&argv[0], &argv[1], &argv[2], &argv[3], &argv[4]);
				if (result != 5)
			...
			if (result == 5) {
				...
				LOG_DEBUG("custom_script aice_read_edm, 0x%x, 0x%x, 0x%x, 0x%x",
						read_edm_reg, read_edm_addr, read_edm_mask, read_edm_exp_data);
				uint32_t get_edm_data = 0;
				i = 0;
				while (1) {
					aice_read_edm_by_coreid(target_id, read_edm_reg, read_edm_addr,
							(uint32_t *)&get_edm_data, 1);
			...
			result = sscanf(curr_str + strlen(compare_str), " %d %d %d", &argv[0], &argv[1], &argv[2]);
			if (result != 3) {
				result = sscanf(curr_str + strlen(compare_str), " 0x%x 0x%x 0x%x",
			...
			if (result == 3) {
				...
				LOG_DEBUG("custom_script aice_read_ctrl, 0x%x, 0x%x, 0x%x",
						read_ctrl_addr, read_ctrl_mask, read_ctrl_exp_data);
...
aice_execute_custom_script_skip_cmd:
		if (i == AICE_CUSTOM_CMD_SET_CURR_TARGET) {
			...
			if (result == 1) {
				...
				LOG_DEBUG("cur_target_name: %s, %s, skip_exe: %d",
						cur_target_name, target_name(target), skip_execute_cmd);
			}
		}
	}
	fclose(script_fd);
...
int aice_reset_aice_as_startup(void)
{
	...
	struct libusb_device_handle *dev = aice_usb_handle;
		...
		uint32_t *pSetData, uint32_t num_of_words, uint32_t attr)
{
	...
	do {
		...
		/* Device to host, receive usb packet from device */
		result = aice_usb_read(pusb_rx_cmmd_info->pusb_buffer, d2h_size);
		if ((result < 0) || (result != (int)d2h_size)) {
				AICE_USBCMMD_MSG("XWRITE, aice_usb_read failed (requested=%d, result=%d)",
						d2h_size, result);
				return ERROR_FAIL;
		}
		aice_pack_usb_cmd(pusb_rx_cmmd_info);
