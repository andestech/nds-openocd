...
unsigned int diagnosis_memory;
unsigned int diagnosis_address;
unsigned int aice_do_diagnosis;
...
unsigned int aice_efreq_value;


/* AICE operations */
static const char *target_cfg_name_str;
#define FILENAME_INPUT     "target/nds32.cfg.tpl"
/*comment out this line, because modified config files must add ICEman log folder(-f $workspacepath)) */
...
uint32_t nds32_reset_halt_as_init;
...
uint32_t nds32_reset_aice_as_startup;
