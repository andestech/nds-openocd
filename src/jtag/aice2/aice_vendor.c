...
uint32_t nds_ftdi_log_detail;
uint32_t nds_ftdi_jtag_opt;
uint32_t sdm_cfg_value;
uint32_t nds32_sdm_core_power_on[32];
uint32_t nds32_sdm_direct_select;
...
unsigned int nds_mixed_mode_checking;
...
static int nds_edm_reg_access_opt(struct target *target, uint32_t ir_scan_value,
		uint32_t addr, uint32_t addr_bits, uint32_t *data_value)
{
	uint32_t in_value1 = 0, in_value2 = 0;
...
nds_edm_reg_access_opt_retry:
	...
	if (nds_ftdi_log_detail) {
		LOG_DEBUG("addr = 0x%x, addr_bits = 0x%x, issue_mask = 0x%x",
				addr, addr_bits, issue_mask);
	}
	jtag_add_dr_scan(target->tap, 2, &field[0], TAP_DRPAUSE);
	jtag_add_runtest(num_clocks, TAP_IDLE);
	jtag_add_dr_scan(target->tap, 2, &field[0], TAP_DRPAUSE);
...
static int nds_edm_reg_access(struct target *target, uint32_t ir_scan_value,
		uint32_t addr, uint32_t addr_bits, uint32_t *data_value)
{
	uint32_t in_value1 = 0, in_value2 = 0;
	uint32_t ir_scan = ir_scan_value;
...
static int nds_ftdi_read_edm(uint32_t core_id, uint8_t JDPInst,
...
static int nds_ftdi_write_edm(uint32_t core_id, uint8_t JDPInst,
		uint32_t address, uint32_t *EDMData, uint32_t num_of_words)
{
	if (nds_ftdi_log_detail)
...
int nds_sdm_check_all_powered_down(uint32_t check_coreid)
{
	...
	for (coreid = 0; coreid < sdm_core_nums; coreid++) {
		...
		nds_ftdi_write_edm(tap->abs_chain_position, ACCESS_MISC_REG,
				NDS_SDM_MISC_PROCESSOR_STATUS, &setValue, 1);

		int retval = nds_ftdi_read_edm(tap->abs_chain_position, ACCESS_MISC_REG,
				NDS_SDM_MISC_PROCESSOR_STATUS, &core_stat, 1);
		if (retval == ERROR_OK) {
