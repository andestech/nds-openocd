		...
		uint32_t reg_no, uint32_t *samples, uint32_t *num_samples)
{
	...
	*num_samples = 0;
	...
	while (total_samples < iteration) {
		...
		if (aice_write_ctrl(AICE_WRITE_CTRL_BATCH_DATA_BUF1_CTRL, 0x40000) != ERROR_OK) {
			retval = ERROR_FAIL;
			goto end_profiling;
		}
		...
		while (!(is_done || is_error || is_break)) {
			...
			if ((0x1f & batch_status) || nds32->gdb_run_mode_halt) { /* batch event or Ctrl+C */
				if (0x10 & batch_status) {
					/* DBGI_EX */
					is_break = true;
				} else if (0x1 & batch_status) {
					/* COMPLETE */
					is_done = true;
				} else if (0xe & batch_status) {
					/* ERROR */
					is_error = true;
				} else {
					is_break = true;
...
/*
		if (this_iteration == NDS32_MAX_PROFILE_ITERATIONS) {
			LOG_DEBUG("this_iteration = %d, prof_total_samples = %d",
...
*/
		...
		if (is_break) {
			/* could be a syscall,
			 * deferred resolving to upper frame */
		} else if (is_complete) {
			/* continue profiling */
			/*
			 ASSERTOK(aice_run_target(target));
...
static int aice_profile_probe_state(struct target *target, enum aice_target_state_s *pstate)
{
	...
	LOG_DEBUG("num=%d, threshold=%d, state=%x",
			nds32->prof_num_samples, nds32->prof_sample_threshold, *pstate);
...
static int nds32_pwr_sampling(void *priv)
{
	...
	if ((p_nds32_pwr_sample_data) &&
		...
		/* for testing */
		/*
		*p_sample_word = nds32->pwr_num_samples;
