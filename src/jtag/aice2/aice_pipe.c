...
static int aice_pipe_read_edm(uint32_t target_id, uint8_t JDPInst, uint32_t address,
		uint32_t *EDMData, uint32_t num_of_words)
{
	char line[AICE_PIPE_MAXLINE];
	char command[AICE_PIPE_MAXLINE];

	/* LOG_DEBUG("AICE Read EDM PIPE: target_id=0x%08X, cmd=0x%02X, addr=0x%08X",
...
static int aice_pipe_write_edm(uint32_t target_id, uint8_t JDPInst, uint32_t address,
		uint32_t *EDMData, uint32_t num_of_words)
{
	char line[AICE_PIPE_MAXLINE];
	char command[AICE_PIPE_MAXLINE];

	/* LOG_DEBUG("AICE Write EDM PIPE: target_id=0x%08X, cmd=0x%02X, addr=0x%08X",
			t arget_id, JDPInst, address); */
...
static int aice_pipe_monitor_command(uint32_t nCmd, char **command, int *len, char **ret_data)
{
	...
	/* Remove first 5 bytes and last 1 byte,
	 * 1st byte->AICE_CUSTOM_MONITOR_CMD,
	 * 2~5 byte->length,
	 * last byte->space
	 **/
	set_u32(send_command+1, now_length-6);
