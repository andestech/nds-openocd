...
static int aice_usb_fastwrite_mem(struct target *target, uint32_t addr,
		const unsigned char *pWriteData, unsigned int num_of_words)
{
...
static int aice_usb_read_mem_h_dim(struct target *target, uint32_t address, uint32_t *data)
{
	...
	if (nds32->data_endian == TARGET_BIG_ENDIAN) {
		value &= 0xFFFF;
		*data = (((value >> 8) & 0xFF) | ((value & 0xFF) << 8));
	} else
		*data = value & 0xFFFF;
	return ERROR_OK;
}
...
static int aice_usb_write_mem_h_dim(struct target *target, uint32_t address, uint32_t data)
{
	...
	if (nds32->data_endian == TARGET_BIG_ENDIAN)
		write_dtrData = (((data >> 8) & 0x00FF) | ((data & 0x00FF) << 8));
	else
		...
		uint8_t *buffer)
{
	...
	while (count > 0) {
		...
		if ((((packet_size*4)+8) % 64) == 0) {
			packet_size -= 1;
			LOG_DEBUG("aice_bulk_write_mem-zero-packet, addr: 0x%08x, packet_size: 0x%08x",
					addr, packet_size);
		}
...
int aice_acc_memory_mode(struct target *target, enum nds_memory_select mem_select)
{
	...
	LOG_DEBUG("aice_usb_memory_mode, memory select: %d (%s)",
			mem_select, (char *)pStringMemSelect[mem_select]);
