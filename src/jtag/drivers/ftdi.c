...
static void ftdi_execute_statemove(struct jtag_command *cmd)
{
	...
	memset(tms_bits, 0xFF, sizeof(tms_bits));
	...
	for (int i = 0; ftdi_vid[i] || ftdi_pid[i]; i++) {
		...
		if (mpsse_ctx) {
			if( ftdi_vid[i] == 0x1cfc ) {
				...
				/* When using multi-tap, the jtag_max_scans should be reduced */
				/*
				...
				if (nds_jtag_max_scans > predict_jtag_max_scans) {
					...
					LOG_DEBUG("Force set nds_jtag_max_scans to predict_jtag_max_scans");
				}
			}
			break;
		}
...
static int ftdi_handle_write_pin_command(Jim_Interp *interp, int argc, Jim_Obj *const *argv)
{
	...
	//jim_wide jim_data;
	//char data;
	//e = Jim_GetOpt_Wide(&goi, &jim_data);
	//if (e != JIM_OK)
	//	return e;
	//data = (char)jim_data&0xF;
	const char *sig_data;
	int sig_data_len;
