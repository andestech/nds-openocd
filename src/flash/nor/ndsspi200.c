...
static int ndsspi200_erase(struct flash_bank *bank, unsigned int first, unsigned int last)
{
	...
	if (target_alloc_working_area(target, sizeof(algorithm_bin),
		...
		if (retval != ERROR_OK) {
			...
			LOG_DEBUG("write code to 0x%" TARGET_PRIxADDR ": 0x%llx bytes",
				nds_algorithm_wa->address, (long long unsigned int)sizeof(algorithm_bin));
	}
...
static int ndsspi200_get_info(struct flash_bank *bank, struct command_invocation *cmd)
{
	struct ndsspi200_flash_bank *ndsspi200_info = bank->driver_priv;
