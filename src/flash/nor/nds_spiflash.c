...
static int ndsspi_get_info(struct flash_bank *bank, struct command_invocation *cmd)
{
	LOG_DEBUG("%s", __func__);
	struct ndsspi_flash_bank *ndsspi_info = bank->driver_priv;
...
static int ndsspi_erase(struct flash_bank *bank, unsigned int first, unsigned int last)
{
	...
	if (strcmp(algo_hdr->magic, TGT_BURN_MAGIC) == 0) {
		...
		if (nds_algorithm_wa == NULL) {
			if (target_alloc_working_area(target, read_size,
				...
				LOG_WARNING("Couldn't allocate %u-byte working area.",
						read_size);
				retval = ERROR_FAIL;
				goto ndsspi_init_finish;
			}
