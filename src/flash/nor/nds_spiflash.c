...
#define 	NDS_AS_DATA_AREA   0x40000  //0x2000//1024//512
#define 	NDS_AS_NUMS        2048     //512
...
bool rv32e = false;
bool tgt_burn_new_version = false;
...
unsigned int nds_spi_rx = 0;
uint32_t custom_flash_device_info_offset = 0;
...
//extern int riscv_xlen(const struct target *target);
...
//extern void riscv_set_current_hartid(struct target *target, int hardid);
...
{
	LOG_DEBUG("%s, base=0x%08x, size=0x%08x", bank->name, bank->base, bank->size);
...
static int ndsspi_probe(struct flash_bank *bank)
{
	...
	LOG_INFO("Found flash device \'%s\' (ID 0x%08x), baseaddr = 0x%08x",
...
ndsspi_write_finish:
	...
	restore_status(bank);
	ndsspi_data_clean(bank);
	//tx is the last step means nds_spi_rx = 0, so delete nds_as and free working area and setting default value to global variable at here
	if (nds_as != NULL)
		nds_as = ndsspi_as_delete(nds_as);
...
int ndsspi_steps_execute(struct algorithm_steps *as, struct flash_bank *bank)
{
	...
	while (!ndsspi_as_empty(as)) {
		...
		/* num_used_reg = 0 for new target burn and (rv32e or v3); num_used_reg = 1 for new target burn and v5 not rv32e; num_used_reg = 2 for v5 older target burn*/
...
int ndsspi_algorithm_apis_init(struct flash_bank *bank)
{
	...
	if(user_algorithm_path == NULL) {
		...
		//char *filename = (char *)NDSSPI_ALGBIN_MAME;
		char filename[2048];
