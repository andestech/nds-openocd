...
struct target *ndsv5_burner_target = NULL;
static int burner_debug_mode = 0;
uint32_t select_burner_coreid = 0;
uint32_t select_burner_rtos_hartid = 0;
...
extern int byte_access_from_burn;
static int burner_fast_byte_readwrite(struct target *target, unsigned char *packet, int *response_len)
{
	...
	LOG_DEBUG("readwrite_addr=0x%08x, readwritecnt=0x%08x, const_addr_mode=%x", readwrite_addr, readwritecnt, nds32->nds_const_addr_mode);
...
static int ndsv5_burner_input(struct connection *connection)
{
	...
	switch (cmd_type)
	{
		...
		case RESET_TARGET:
			...
			retval = target_resume(target, 1, 0, 0, 0);	//free run in debug mode
...
static int ndsv5_burner_new_connection(struct connection *connection)
{
	...
	struct target *target;
	for (target = all_targets; target; target = target->next) {
		// setting ndsv5_burner_target:fix bug for ndsv5_burner_input failed, but restart SPI_burn/PAR_burn to try again
		if (!first_target) {
...
static uint32_t ndsv5_burner_server_init_done = 0;
