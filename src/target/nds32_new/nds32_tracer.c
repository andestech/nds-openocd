...
unsigned long nds32_chk_tracer_xrw_support;
unsigned long nds32_otb_id, nds32_tdrx_id;

FILE *pOTBPacketFile;
unsigned long nds32_etb_ori_fifo;
unsigned long nds32_otb_filesize = TRACER_OTB_FILESIZE;
unsigned long read_otb_already, write2file_already = 1;
unsigned long nds32_tracer_backup_file;
unsigned long nds32_tracer_backup_filesize;
unsigned long nds32_tracer_on;
unsigned long nds32_etb_id, nds32_etm_id, nds32_etb_depth, nds32_etb_revision;
unsigned long nds32_capture_cnt, nds32_trigger_ctrl, nds32_realtime_mode = 1;
unsigned long nds32_sync_period = ETM_SYNC_CTL_PERIOD_1KB, nds32_etm_mode, nds32_etb_mode;
unsigned long *pnds32_etb_buf_start, *pnds32_etb_buf_end;
unsigned long *pnds32_etb_wptr;
...
unsigned long nds32_cycle_accurate;
unsigned long nds32_ras_depth;
...
static int nds32_tracer_setting(struct target *target)
{
	...
	if (nds32_trigger_ctrl == (ETM_TRIGGER_EVENT_A_TRUE | ETM_TRIGGER_A_XOR_B)) {
		/* non-trigger */
...
unsigned long sync_pkt_counter;
unsigned long last_sync_pkt_length;
unsigned long sync_pkt_counter_prev;
...
static int nds32_tracer_dumpfile(struct target *target, char *pFileName)
{
	...
	if (nds32_skip_find_1stsync == 0) {
		...
		dump_pkt_bytes = nds32_tracer_search_1stsync_lastsync(pbuf_start, total_pkt_bytes, &p1st_sync);
		LOG_DEBUG("pbuf_start = 0x%lx, p1st_sync = 0x%lx, dump_pkt_bytes = 0x%lx",
				(unsigned long)pbuf_start, (unsigned long)p1st_sync, dump_pkt_bytes);
...
static int nds32_tracer_search_1stsync_lastsync(char *pSrcPkt, unsigned long total_pkt_bytes, char **p1stPkt)
{
	...
	while (total_pkt_bytes) {
		...
		if ((gp_curr_sync) && (find_1st_sync == 0)) {
			find_1st_sync = 1;
