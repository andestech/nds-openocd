	...
	switch (AiceCmmdType) {
		...
		case MONITOR_CMD:
			...
			if (retval == ERROR_OK) {
				...
				/*copy data including length(ret_size:4Bytes) */
				memcpy(buf_p + 2, ret_data, 4 + ret_size);

				/* buf_p[0]=CMD, buf_p[1]=0, buf_p[2~...]=data */
				res_length = 2 + 4 + ret_size;
				free(ret_data);
			} else {
...
static int burner_new_connection(struct connection *connection)
{
	...
	LOG_DEBUG("access_channel 0x%lx, 0x%lx",
			(long)&nds32->memory.access_channel, (long)nds32->memory.access_channel);
	LOG_DEBUG("access_control 0x%lx, direct_access_local_memory 0x%lx",
			(long)nds32->edm.access_control, (long)nds32->edm.direct_access_local_memory);
...
static const struct service_driver burner_service_driver = {
	.name = "burner",
	.new_connection_during_keep_alive_handler = NULL,
	.new_connection_handler = burner_new_connection,
	.input_handler = burner_input,
	.connection_closed_handler = burner_connection_closed,
	.keep_client_alive_handler = NULL,
};

static unsigned int burner_server_init_done;
int burner_server_init(char *banner)
{
	...
	if (burner_server_init_done)
		return ERROR_OK;
	ret_value = add_service(&burner_service_driver, burner_port, 1, NULL);
