...
#include <jtag/aice2/aice_apis.h>
#include "nds32_log.h"
...
#endif
uint32_t nds32_dis_global_stop_warning;
uint32_t nds32_custom_def_idlm_base;
uint32_t nds32_security_compat_display;
...
COMMAND_HANDLER(handle_nds32_set_watch_command)
{
	...
	if (CMD_ARGC == 0) {
		...
		while (watchpoint) {
			...
			LOG_DEBUG("nds-watchpoint address: 0x%" TARGET_PRIxADDR ", length: 0x%08x",
...
static const char * const run_mode_names[] = {
...
COMMAND_HANDLER(handle_nds32_configure_command)
{
	...
	} else if (PROPERTY_CAN_PROFILE == property) {
...
static const char * const bench_item_names[] = {
