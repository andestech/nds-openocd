...
uint32_t nds32_backup_psw;
uint32_t nds32_use_psw_mode;
#endif
...
static int nds32_get_core_reg_64(struct reg *reg)
{
	...
	if (reg_arch_info->enable == false) {
		...
		if ((nds32->fpu_enable == false && NDS32_REG_TYPE_FPU == reg_type) ||
		    (nds32->cop_enable[0] == false && NDS32_REG_TYPE_COP0 == reg_type) ||
		    (nds32->cop_enable[1] == false && NDS32_REG_TYPE_COP1 == reg_type) ||
		    (nds32->cop_enable[2] == false && NDS32_REG_TYPE_COP2 == reg_type) ||
		    (nds32->cop_enable[3] == false && NDS32_REG_TYPE_COP3 == reg_type)) {
...
static int nds32_update_psw(struct nds32 *nds32)
{
	...
	/* LOG_DEBUG("address_translation: %d, va_to_pa_off: %d",
			(int)nds32->memory.address_translation, nds32->memory.va_to_pa_off); */
	return ERROR_OK;
}
...
static int nds32_set_core_reg(struct reg *reg, uint8_t *buf)
{
	...
	if ((nds32->fpu_enable == false && NDS32_REG_TYPE_FPU == reg_type) ||
	    (nds32->audio_enable == false && NDS32_REG_TYPE_AUMR == reg_type) ||
	    (nds32->cop_enable[0] == false && NDS32_REG_TYPE_COP0 == reg_type) ||
	    (nds32->cop_enable[1] == false && NDS32_REG_TYPE_COP1 == reg_type) ||
	    (nds32->cop_enable[2] == false && NDS32_REG_TYPE_COP2 == reg_type) ||
	    (nds32->cop_enable[3] == false && NDS32_REG_TYPE_COP3 == reg_type)) {
		...
		struct nds32 *nds32)
{
	...
	struct nds32_reg *reg_arch_info =
		calloc(TOTAL_REG_NUM + total_ace_reg_nums + total_cop_reg_nums, sizeof(struct nds32_reg));
...
uint32_t curr_mem_access_attr_index;
...
int nds32_virtual_to_physical(struct target *target, target_addr_t address, target_addr_t *physical)
{
	...
	if ((nds32->memory.address_translation == false) ||
		...
		*physical = address;
		LOG_DEBUG("va_to_pa_off:%d, address_translation:%d",
				nds32->memory.va_to_pa_off, nds32->memory.address_translation);
		return ERROR_OK;
	}
...
int nds32_cache_sync(struct target *target, target_addr_t address, uint32_t length)
{
	...
	for (cur_address = address, cur_line = start_line; cur_line <= end_line;
...
int nds32_log_callback(void *priv)
{
	...
	if ((FileSize >= MaxLogFileSize) || (pLogFile == NULL)) {
		if (pLogFile)
			fclose(pLogFile);
		LogFileIdx ^= 0x01;
...
unsigned int nds32_do_once;
unsigned int nds32_set_log_callback;
...
int nds32_get_gdb_fileio_info(struct target *target, struct gdb_fileio_info *fileio_info)
{
	...
	switch (nds32->active_syscall_id) {
		...
		case NDS32_SYSCALL_FREOPEN:
			{
				...
				fileio_info->identifier = "freopen";
				nds32_get_mapped_reg(nds32, R0, (uint32_t *)&(fileio_info->param_1));
				/* reserve fileio_info->param_2 for length of path */
				nds32_get_mapped_reg(nds32, R1, (uint32_t *)&(fileio_info->param_3));
				/* reserve fileio_info->param_4 for length of mode */
			...
			break;
		case NDS32_SYSCALL_FCLOSE:
			fileio_info->identifier = "fclose";
			nds32_get_mapped_reg(nds32, R0, (uint32_t *)&(fileio_info->param_1));
			break;
		case NDS32_SYSCALL_FFLUSH:
			fileio_info->identifier = "fflush";
			nds32_get_mapped_reg(nds32, R0, (uint32_t *)&(fileio_info->param_1));
			break;
		case NDS32_SYSCALL_FREAD:
			fileio_info->identifier = "fread";
			nds32_get_mapped_reg(nds32, R0, (uint32_t *)&(fileio_info->param_1));
			nds32_get_mapped_reg(nds32, R1, (uint32_t *)&(fileio_info->param_2));
			nds32_get_mapped_reg(nds32, R2, (uint32_t *)&(fileio_info->param_3));
		...
		case NDS32_SYSCALL_FWRITE:
			...
			break;
		case NDS32_SYSCALL_FGETC:
			fileio_info->identifier = "fgetc";
			nds32_get_mapped_reg(nds32, R0, (uint32_t *)&(fileio_info->param_1));
			break;
		case NDS32_SYSCALL_FGETS:
			fileio_info->identifier = "fgets";
			nds32_get_mapped_reg(nds32, R0, (uint32_t *)&(fileio_info->param_1));
			nds32_get_mapped_reg(nds32, R1, (uint32_t *)&(fileio_info->param_2));
			nds32_get_mapped_reg(nds32, R2, (uint32_t *)&(fileio_info->param_3));
			break;
		case NDS32_SYSCALL_FPUTC:
			fileio_info->identifier = "fputc";
			nds32_get_mapped_reg(nds32, R0, (uint32_t *)&(fileio_info->param_1));
			nds32_get_mapped_reg(nds32, R1, (uint32_t *)&(fileio_info->param_2));
			break;
		case NDS32_SYSCALL_FPUTS:
			fileio_info->identifier = "fputs";
			nds32_get_mapped_reg(nds32, R0, (uint32_t *)&(fileio_info->param_1));
			nds32_get_mapped_reg(nds32, R1, (uint32_t *)&(fileio_info->param_2));
			break;
		case NDS32_SYSCALL_UNGETC:
			fileio_info->identifier = "ungetc";
			nds32_get_mapped_reg(nds32, R0, (uint32_t *)&(fileio_info->param_1));
			nds32_get_mapped_reg(nds32, R1, (uint32_t *)&(fileio_info->param_2));
			break;
		case NDS32_SYSCALL_FTELL:
			fileio_info->identifier = "ftell";
			nds32_get_mapped_reg(nds32, R0, (uint32_t *)&(fileio_info->param_1));
			break;
		case NDS32_SYSCALL_FSEEK:
			fileio_info->identifier = "fseek";
			nds32_get_mapped_reg(nds32, R0, (uint32_t *)&(fileio_info->param_1));
			nds32_get_mapped_reg(nds32, R1, (uint32_t *)&(fileio_info->param_2));
			nds32_get_mapped_reg(nds32, R2, (uint32_t *)&(fileio_info->param_3));
			break;
		case NDS32_SYSCALL_REWIND:
			fileio_info->identifier = "rewind";
			nds32_get_mapped_reg(nds32, R0, (uint32_t *)&(fileio_info->param_1));
			break;
		case NDS32_SYSCALL_CLEARERR:
			fileio_info->identifier = "clearerr";
			nds32_get_mapped_reg(nds32, R0, (uint32_t *)&(fileio_info->param_1));
			break;
		case NDS32_SYSCALL_FEOF:
			fileio_info->identifier = "feof";
			nds32_get_mapped_reg(nds32, R0, (uint32_t *)&(fileio_info->param_1));
			break;
		case NDS32_SYSCALL_FERROR:
			fileio_info->identifier = "ferror";
			nds32_get_mapped_reg(nds32, R0, (uint32_t *)&(fileio_info->param_1));
			break;
		case NDS32_SYSCALL_REMOVE:
			{
				uint8_t filename[256];
				fileio_info->identifier = "remove";
				nds32_get_mapped_reg(nds32, R0, (uint32_t *)&(fileio_info->param_1));
				/* reserve fileio_info->param_2 for length of path */
				target->type->read_buffer(target, fileio_info->param_1,
						256, filename);
			...
			break;
		case NDS32_SYSCALL_TMPFILE:
			fileio_info->identifier = "tmpfile";
			break;
		default:
