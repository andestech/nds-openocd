...
static int riscv_examine(struct target *target)
{
	...
	uint32_t dmstatus = dmi_read(target, DM_DMSTATUS);
	if (get_field(dmstatus, DM_DMSTATUS_VERSION) != 2) {
		LOG_ERROR("OpenOCD only supports Debug Module version 2, not %d "
				"(dmstatus=0x%x)", get_field(dmstatus, DM_DMSTATUS_VERSION), dmstatus);
		return ERROR_FAIL;
	}
	...
	dmstatus = dmi_read(target, DM_DMSTATUS);

	dmi_write(target, DM_DMCONTROL, DM_DMCONTROL_DMACTIVE);
	uint32_t dmcontrol = dmi_read(target, DM_DMCONTROL);
	...
	info->sbcs = dmi_read(target, DM_SBCS);
	if (info->sbcs & 0x1F) {
		/** DM_SBCS_SBACCESS8|DM_SBCS_SBACCESS16|DM_SBCS_SBACCESS32|
		    DM_SBCS_SBACCESS64|DM_SBCS_SBACCESS128 **/
	...
	uint32_t abstractcs = dmi_read(target, DM_ABSTRACTCS);
...
static void decode_dmi(char *text, unsigned address, unsigned data)
{
	static const struct {
		...
		{ DM_DMCONTROL, DM_DMCONTROL_HARTSELLO, "hartsel" },
		...
		{ DM_COMMAND, DM_COMMAND_CONTROL, "control" },
		...
		uint32_t size, uint8_t *buffer)
{
	uint32_t value;
	if (size > 12) {
		/* if (dmi_read(target, &value, DM_SBDATA3) != ERROR_OK)
			return ERROR_FAIL; */
		value = dmi_read(target, DM_SBDATA3);
		write_to_buf(buffer + 12, value, 4);
		log_memory_access(address + 12, value, 4, true);
	}
	if (size > 8) {
		/* if (dmi_read(target, &value, DM_SBDATA2) != ERROR_OK)
			return ERROR_FAIL; */
		value = dmi_read(target, DM_SBDATA2);
		write_to_buf(buffer + 8, value, 4);
		log_memory_access(address + 8, value, 4, true);
	}
	if (size > 4) {
		/*if (dmi_read(target, &value, DM_SBDATA1) != ERROR_OK)
			return ERROR_FAIL; */
		value = dmi_read(target, DM_SBDATA1);
		write_to_buf(buffer + 4, value, 4);
		log_memory_access(address + 4, value, 4, true);
	}
	/* if (dmi_read(target, &value, DM_SBDATA0) != ERROR_OK)
		return ERROR_FAIL; */
	value = dmi_read(target, DM_SBDATA0);
...
static target_addr_t sb_read_address(struct target *target)
{
	...
	if (sbasize > 32) {
		...
		v = dmi_read(target, DM_SBADDRESS1);
		address |= v;
		address <<= 32;
	}
	/* dmi_read(target, &v, DM_SBADDRESS0); */
	v = dmi_read(target, DM_SBADDRESS0);
	address |= v;
	return address;
}
