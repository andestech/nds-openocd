...
static void write_gmon_1(struct nds32_v5 *nds32, uint32_t num_samples)
{
	...
	for (uint32_t i = 0; i < num_samples; i++) {
		...
		//	(long unsigned int)address, (long unsigned int)nds32->prof_addr_min, (long unsigned int)nds32->prof_addr_max);
...
static int ndsv5_profile_probe_pc(void *priv)
{
	...
	NDS_INFO("probe_pc...");
#if _NDS_MEM_Q_ACCESS_
	//if ((nds_dmi_quick_access) && (riscv_debug_buffer_size(target) >= 6)) {
